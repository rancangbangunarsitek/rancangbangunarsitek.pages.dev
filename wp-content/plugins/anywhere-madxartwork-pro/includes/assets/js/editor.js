jQuery(document).ready(function () {
	...
	madxartwork.hooks.addAction('panel/open_editor/widget/ae-post-blocks-adv', get_ae_widget_id);
	madxartwork.hooks.addAction('panel/open_editor/widget/ae-post-blocks', get_ae_widget_id);
	madxartwork.hooks.addAction('panel/open_editor/widget/ae-acf-repeater', get_ae_widget_id);
	madxartwork.hooks.addAction('panel/open_editor/widget/ae-acf-gallery', get_ae_widget_id);
	madxartwork.hooks.addAction('panel/open_editor/widget/ae-acf-flexible-content', get_ae_widget_id);
	...
	madxartwork.hooks.addAction('panel/open_editor/widget', function (panel, model, view) {
        ...
        if(widget_type == 'ae-acf'){
            jQuery('.madxartwork-control-_skin label').html('Field Type');
        }
		...
		if (widget_type == 'ae-post-blocks-adv') {
			jQuery('.madxartwork-control-_skin label').html('Layout');
...
(function (madxartwork, $, window) {
	// Query Control
	console.log(madxartwork.modules.controls);
	var Raw_Html = madxartwork.modules.controls.Wysiwyg.extend({
	...
	madxartwork.addControlView( 'aep-raw-html', Raw_Html );

	var Query = madxartwork.modules.controls.Select2.extend({
		cache: null,
		...
		getSelect2DefaultOptions: function getSelect2DefaultOptions() {
			...
			console.log('Self Modal', self.model);
			...
			return jQuery.extend(
				madxartwork.modules.controls.Select2.prototype.getSelect2DefaultOptions.apply(
					this,
					arguments
				),
				{
					ajax: {
						transport: function transport(params, success, failure) {
							...
							if(self.model.get("query_type") === 'flex-sub-fields'){
								...
								console.log('Settings =---->', self.elementSettingsModel);
								console.log('Parent Field =---->', parent_field);
								if(typeof self.elementSettingsModel.attributes[parent_field] !== "undefined"){
									data.flex_parent_field = self.elementSettingsModel.attributes[parent_field];
								}
							...
							if(self.model.get("query_type") === 'repeater-sub-fields'){
								...
								data.repeater_parent_field = self.elementSettingsModel.attributes[parent_field];
							}

							return madxartworkCommon.ajax.addRequest(
		...
		getValueTitles: function getValueTitles() {
			...
			if(queryType === 'flex-sub-fields'){
				var flexParentField = self.elementSettingsModel.attributes[parent_field];
			}
			...
			if(queryType === 'repeater-sub-fields'){
				var repeaterField = self.elementSettingsModel.attributes[parent_field];
			}
		...
		addSpinner: function addSpinner() {
			...
			this.$el
				.find(".madxartwork-control-title")
				.after(
					'<span class="madxartwork-control-spinner pp-control-spinner">&nbsp;<i class="fa fa-spinner fa-spin"></i>&nbsp;</span>'
				);
		},

		onReady: function onReady() {
			setTimeout(
				madxartwork.modules.controls.Select2.prototype.onReady.bind(this)
			);
	...
	madxartwork.addControlView( 'aep-query', Query );
