...
/* global madxartworkScreenshotConfig */
class Screenshot extends madxartworkModules.ViewModule {
  getDefaultSettings() {
    return {
      ...
      timerLabel: null,
      timer_label: `${madxartworkScreenshotConfig.post_id} - timer`,
      image_placeholder: 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs=',
      isDebug: madxartworkCommonConfig.ismadxartworkDebug,
      isDebugSvg: false,
      ...madxartworkScreenshotConfig
    };
  }
  getDefaultElements() {
    const $madxartwork = jQuery(madxartworkScreenshotConfig.selector);
    const $sections = $madxartwork.find('.madxartwork-section-wrap > .madxartwork-section, .madxartwork > .madxartwork-section');
    return {
      $madxartwork,
      $sections,
      $firstSection: $sections.first(),
      $notmadxartworkElements: madxartworkCommon.elements.$body.find('> *:not(style, link)').not($madxartwork),
      $head: jQuery('head')
    };
  }
  onInit() {
   ...
   */
  captureScreenshot() {
    if (!this.elements.$madxartwork.length) {
      madxartworkCommon.helpers.consoleWarn('Screenshots: The content of this page is empty, the module will create a fake conent just for this screenshot.');
   ...
   */
  createFakeContent() {
    this.elements.$madxartwork = jQuery('<div>').css({
    ...
    this.elements.$madxartwork.append(jQuery('<h1>').css({
      fontSize: '85px'
    }).html(this.getSettings('empty_content_headline')));
    document.body.prepend(this.elements.$madxartwork);
  }
   ...
   */
  handleIFrames() {
    this.elements.$madxartwork.find('iframe').each((index, el) => {
  ...
  removeUnnecessaryElements() {
    ...
    this.elements.$notmadxartworkElements.remove();
  }
   ...
   */
  handleLinks() {
    madxartworkCommon.elements.$body.find('a').attr('href', '/');
  }
  ...
  download(uri) {
    ...
    madxartworkCommon.elements.$body.append($link);
    $link.trigger('click');
  }
  ...
  save(canvas) {
    return new Promise((resolve, reject) => {
      madxartworkCommon.ajax.addRequest('screenshot_save', {
        data: {
  ...
  markAsFailed() {
    return new Promise((resolve, reject) => {
      madxartworkCommon.ajax.addRequest('screenshot_failed', {
        data: {
          post_id: this.getSettings('post_id')
        },
        success: () => {
