	...
	function ( $ ) {
		var WidgetElements_SvgDistortionHandler = function ( $scope,$ ) {
			...
			if ( madxartworkFrontend.isEditMode() ) {
				...
				$( '.madxartwork-element[data-id=' + id_scope + '] svg, .' + elementSettings.id_svg_class +
				   ' a' ).off( 'mouseenter' );
				$( '.madxartwork-element[data-id=' + id_scope + '] svg, .' + elementSettings.id_svg_class +
				   ' a' ).off( 'mouseleave' );
				$( '.madxartwork-element[data-id=' + id_scope + '] svg, .' + elementSettings.id_svg_class +
			...
			if ( elementSettings.svg_trigger == 'animation' ) {
				...
				if ( madxartworkFrontend.isEditMode() ) playShapeEl();
		...
		$( window ).on( 'madxartwork/frontend/init',function () {
			madxartworkFrontend.hooks.addAction( 'frontend/element_ready/dyncontel-svgdistortion.default',WidgetElements_SvgDistortionHandler );
		} );
	}
)( jQuery );
