...
(function ($) {
    var WidgetDyncontel_ACFRepeaterHandler = function ($scope, $) {
        ...
        if (elementSettings.dce_acf_repeater_format == 'accordion') {
            $scope.find('.madxartwork-tab-title').on('click', function(){
                if (elementSettings.dce_acf_repeater_accordion_close) {
                    $scope.find('.madxartwork-active').not(this).each(function() {
                        jQuery(this).toggleClass('madxartwork-active');
                        jQuery(this).next().slideToggle().toggleClass('madxartwork-active');
                    });
                }
                jQuery(this).toggleClass('madxartwork-active');
                jQuery(this).next().slideToggle().toggleClass('madxartwork-active');
        ...
        if (elementSettings.dce_acf_repeater_format == 'masonry') {
            ...
            var swiperOptions = {
                ...
                loop: cicloInfinito, 
                navigation: {
                    nextEl: id_post ? '.dce-madxartwork-post-'+id_post+' .madxartwork-element-' + id_scope + ' .next-' + counter_id : '.next-' + counter_id,
                    prevEl: id_post ? '.dce-madxartwork-post-'+id_post+' .madxartwork-element-' + id_scope + ' .prev-' + counter_id : '.prev-' + counter_id,
                },
                pagination: {
                    el: id_post ? '.dce-madxartwork-post-'+id_post+' .madxartwork-element-' + id_scope + ' .pagination-' + counter_id : '.pagination-' + counter_id, //'.swiper-pagination', //'.pagination-acfslider-'+id_scope,
                    clickable: true,
...

			{
				...
				if (!isCarousel) {
					...
					let madxartworkTab = elementSwiper.closest('.madxartwork-tab-content');
					if (madxartworkTab && elementSettings.useAutoplay) { // we are inside a tab, autoplay needs to be updated.
						manageAutoplayInsideTab( madxartworkTab, swiperOptions, elementSwiper );
					} else {
						initializeSwiper(swiperOptions, elementSwiper);
					}
				}
    ...
    $(window).on('madxartwork/frontend/init', function () {
        madxartworkFrontend.hooks.addAction('frontend/element_ready/dce-acf-repeater-v2.default', WidgetDyncontel_ACFRepeaterHandler);
    });
