...
setWidth(){heightAdminBar=0;if($('body').is('.admin-bar')&&!madxartworkFrontend.isEditMode()){heightAdminBar=45}
...
setHeight(){heightAdminBar=0;if($('body').is('.admin-bar')&&!madxartworkFrontend.isEditMode()){heightAdminBar=45}
...
newPagination+='<li><a href="#'+$(this).attr("data-id")+'" class="'+activeClass+' nav__item" aria-label="'+(i+1)+'">'+$itemInner+$itemTitle+'</a></li>'}});newPagination+="</ul>";$("body").append(newPagination)};var init_ScrollEffects=function(){$('body').addClass('dce-pageScroll dce-scrolling');if(settings_page.custom_class_section){$customClass=settings_page.custom_class_section}else{$customClass='madxartwork-section'}
$target_sections='.madxartwork-'+currentPostId;if(!$target_sections)
$target_sections='';sezioni=$target_sections+'.madxartwork .madxartwork-section-wrap > .'+$customClass;sectionsAvailable=$(sezioni);wrapperSezioni=$($target_sections+'.madxartwork .madxartwork-section-wrap');$($target_sections).addClass('scroll-direction-'+settings_page.directionScroll);animationType=settings_page.animation_effects||['spin'];var animationType_string=[];if(animationType.length)
...
lax.update(window.scrollY);requestId=window.requestAnimationFrame(updateLax)};requestId=window.requestAnimationFrame(updateLax);is_scrollEffects=!0};var init_InertiaScroll=function(){if(settings_page.custom_class_section){$customClass=settings_page.custom_class_section}else{$customClass='madxartwork-section'}
...
$target_sections='';sezioni=$target_sections+'.madxartwork .madxartwork-section-wrap .'+$customClass;sectionsAvailable=$(sezioni);wrapperSezioni=$($target_sections+'.madxartwork .madxartwork-section-wrap');if($('.madxartwork-template-canvas').length){main=document.querySelector($target_sections);mainWrap=document.querySelector($target_sections+'.madxartwork .madxartwork-section-wrap')}else{if(settings_page.automatic_wrapper){if(!$('#outer-wrap').length)
...
setTimeout(function(){if(settings_page.enable_inertiaScroll)
init_InertiaScroll()},100)}else{removeInertiaScroll()}}
$(window).on('madxartwork/frontend/init',function(){if(typeof madxartworkFrontendConfig.settings.page!=='undefined'){settings_page=madxartworkFrontendConfig.settings.page;currentPostId=madxartworkFrontendConfig.post.id;is_enable_dceScrolling=settings_page.enable_dceScrolling;is_enable_scrollify=settings_page.enable_scrollify;is_enable_scrollEffects=settings_page.enable_scrollEffects;is_enable_inertiaScroll=settings_page.enable_inertiaScroll;var responsive_scrollEffects=settings_page.responsive_scrollEffects;var responsive_snapScroll=settings_page.responsive_snapScroll;var responsive_inertiaScroll=settings_page.responsive_inertiaScroll;var deviceMode=$('body').attr('data-madxartwork-device-mode');if(is_enable_scrollEffects&&is_enable_dceScrolling&&$.inArray(deviceMode,responsive_scrollEffects)>=0){init_ScrollEffects()}
