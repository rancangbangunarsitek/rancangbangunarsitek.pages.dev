( function( $ ) {
	...
	var WidgetElements_SvgMorphHandler = function( $scope, $ ) {
		...
		*/
		...
		var createTween = function(){
			...
			is_running = true;
			if( run == 'paused' && madxartworkFrontend.isEditMode() ){
				ferma();
			}
		...
		var active_scrollAnalysi = function($el){
			if($el){
				...
				madxartworkFrontend.waypoint($($el), runAnim, waypointOptions);
			}
		};
		...
		if(!madxartworkFrontend.isEditMode() && contentElemsTotal > 1 && elementSettings.svg_trigger == 'animation'){
		...
		if(madxartworkFrontend.isEditMode()){
			if(transitionTl) transitionTl.kill($(dceshape));
			if(transitionTlpos) transitionTlpos.kill($(dceshape_svg));
			$('.madxartwork-element[data-id='+id_scope+']').off('mouseenter','svg');
			$('.madxartwork-element[data-id='+id_scope+']').off('mouseleave','svg');
			$('.madxartwork-element[data-id='+id_scope+']').off('touchstart','svg');
			$('.madxartwork-element[data-id='+id_scope+']').off('touchend','svg');
		}
		...
		setTimeout(function(){
			...
			if(elementSettings.svg_trigger == 'animation'){
				...
				$('.madxartwork-element[data-id='+id_scope+']').on('mouseenter','svg', mouseenterFn);
				$('.madxartwork-element[data-id='+id_scope+']').on('mouseleave','svg', mouseleaveFn);
				$('.madxartwork-element[data-id='+id_scope+']').on('touchstart','svg', mouseenterFn);
				$('.madxartwork-element[data-id='+id_scope+']').on('touchend','svg', mouseleaveFn);
	...
	$( window ).on( 'madxartwork/frontend/init', function() {
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/dyncontel-svgmorphing.default', WidgetElements_SvgMorphHandler );
	} );
} )( jQuery );
