( function( $ ) {
	...
	var WidgetElements_RevealHandler = function( $scope, $ ) {
      ...
      if (!jQuery(revealId).hasClass('block-revealer')) {
          revealAction();
			...
			madxartworkFrontend.waypoint($(revealistance), runReveal, waypointOptions);
	...
	$( window ).on( 'madxartwork/frontend/init', function() {
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/global', WidgetElements_RevealHandler );
	} );
} )( jQuery );
