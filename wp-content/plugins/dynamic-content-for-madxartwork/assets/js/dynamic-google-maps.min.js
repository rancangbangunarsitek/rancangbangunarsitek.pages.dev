...
if(elementSettingsMap.markerclustererControl){new MarkerClusterer(map,markers,{imagePath:'/wp-content/plugins/dynamic-content-for-madxartwork/assets/lib/gmap/markerclusterer/img/m'})}}else{if(mapDataType=='address'){addressToLocation(indirizzo,imageMarker,infoWindow,null,changeMapLocation)}else if(mapDataType=='latlng'){var latLng=new google.maps.LatLng(lati,long);map.panTo(latLng);var infoWindowMap=new google.maps.InfoWindow({content:infoWindow});var marker=new google.maps.Marker({position:latLng,map:map,icon:imageMarker,animation:google.maps.Animation.DROP,});if(elementSettingsMap.enable_infoWindow){marker.addListener('click',function(){infoWindowMap.open(map,this)})}}}}
function changeMapLocation(locations){if(locations&&locations.length>=1){var image={url:locations[0].marker,};var objMarker={map:map,position:locations[0].location,};if(locations[0].marker!=""){objMarker.icon=image}
var marker=new google.maps.Marker(objMarker);var infoWindowMap=new google.maps.InfoWindow({content:locations[0].infoWindow,});map.panTo(locations[0].location);if(elementSettingsMap.enable_infoWindow){marker.addListener('click',function(){if(elementSettingsMap.infoWindow_click_to_post){if(isEditMode){alert('You clicked: '+locations[0].postLink);return!1}else{window.location=locations[0].postLink}}else{infoWindowMap.open(map,marker)}})}
if(elementSettingsMap.use_query){bounds.extend(marker.position);map.fitBounds(bounds)}}}};function addressToLocation(address,markerimg,iw,pl,callback){var geocoder=new google.maps.Geocoder();geocoder.geocode({address:address},function(results,status){var resultLocations=[];if(status==google.maps.GeocoderStatus.OK){if(results){var numOfResults=results.length;for(var i=0;i<numOfResults;i++){var result=results[i];resultLocations.push({text:result.formatted_address,addressStr:result.formatted_address,location:result.geometry.location,marker:markerimg,postLink:pl,infoWindow:iw})}}}
...
$(window).on('madxartwork/frontend/init',function(){if(madxartworkFrontend.isEditMode()){isEditMode=!0}
if($('body').is('.admin-bar')){isAdminBar=!0}
madxartworkFrontend.hooks.addAction('frontend/element_ready/dyncontel-acf-google-maps.default',DyncontEl_GoogleMapsHandler)})})(jQuery);(function($){"use strict";$(function(){$(document).on("sf:ajaxfinish",".searchandfilter",function(e,data){if(madxartworkFrontend){if(madxartworkFrontend.elementsHandler.runReadyTrigger&&SF_LDATA.extensions.indexOf('search-filter-madxartwork')<0){madxartworkFrontend.elementsHandler.runReadyTrigger(data.targetSelector)}}})})}(jQuery))