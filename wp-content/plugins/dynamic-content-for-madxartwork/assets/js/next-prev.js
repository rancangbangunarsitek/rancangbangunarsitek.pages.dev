(function ($) {
    var WidgetElements_NextPrevHandler = function ($scope, $) {
        if (typeof $('#main').ajaxify === "function") {
            $('#main').ajaxify({
                ...
                selector: ".madxartwork-widget-dyncontel-post-nextprev a:not(.no-ajaxy)", //Selector for elements to ajaxify - without being swapped - e.g. a selection of links
    ...
    $(window).on('pronto.render', function (event, eventInfo) {
        ...
        if (typeof eventInfo.delegateTarget !== 'undefined') {
            var element_el = $(eventInfo.delegateTarget.innerHTML).find('.madxartwork-element');
        ...
        $(window).trigger('madxartwork/frontend/init');
        $(window).trigger('ready');

    });
    // Make sure you run this code under madxartwork..
    $(window).on('madxartwork/frontend/init', function () {
        madxartworkFrontend.hooks.addAction('frontend/element_ready/dyncontel-post-nextprev.default', WidgetElements_NextPrevHandler);
    });
})(jQuery);
