...
function initializeAllJsFieldFields($scope) {
	$scope.find('.madxartwork-field-type-dce_js_field').each((_, w) => initializeJsField(w, $scope[0]));
}
...
jQuery(window).on('madxartwork/frontend/init', function() {
	...
	madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', initializeAllJsFieldFields);
});
