...
function initializeAllPayPalButtons($scope) {
	$scope.find('.madxartwork-field-type-dce_form_paypal').each((_, w) => initializePayPalButtons(w, $scope));
}

jQuery(window).on('madxartwork/frontend/init', function() {
	madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', initializeAllPayPalButtons);
});
