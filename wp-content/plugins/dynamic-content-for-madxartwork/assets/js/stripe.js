...
function initializeStripeField(wrapper, $scope) {
	...
	const $submitButton = $scope.find('.madxartwork-field-type-submit button');
...
function initializeAllStripeFields($scope) {
	$scope.find('.madxartwork-field-type-dce_form_stripe').each((_, w) => initializeStripeField(w, $scope));
}

jQuery(window).on('madxartwork/frontend/init', function() {
	madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', initializeAllStripeFields);
});
