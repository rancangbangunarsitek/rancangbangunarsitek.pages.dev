(function($){var WidgetElements_PdfViewer=function($scope,$){let scope=$scope[0];var elementSettings=get_Dyncontel_ElementSettings($scope);var url='';if('url'===elementSettings.source&&elementSettings.source_url.url){url=elementSettings.source_url.url}else if('media_file'===elementSettings.source&&elementSettings.source_media){url=elementSettings.source_media}
if(!url){return}
...
function render(){myState.pdf.getPage(myState.currentPage).then((page)=>{var canvas=scope.getElementsByClassName("dce-pdf-renderer")[0];var ctx=canvas.getContext('2d');var viewport=page.getViewport({scale:myState.zoom});canvas.width=viewport.width;canvas.height=viewport.height;page.render({canvasContext:ctx,viewport:viewport})})}};$(window).on('madxartwork/frontend/init',function(){madxartworkFrontend.hooks.addAction('frontend/element_ready/dce-pdf-viewer.default',WidgetElements_PdfViewer)})})(jQuery)