...
const initializeDynamicSelectFields = ($form) => {
	$form.find('.madxartwork-field-type-dynamic_select').each((_, f) => initializeDynamicSelectField(f, $form[0]));
};

jQuery(window).on('madxartwork/frontend/init', function() {
	madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', initializeDynamicSelectFields);
});
