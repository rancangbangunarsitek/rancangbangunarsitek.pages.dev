jQuery( window ).on( 'madxartwork/frontend/init', () => {
	class OsmMap extends madxartworkModules.frontend.handlers.Base {
 		getDefaultSettings() {
 			return {
 				selectors: {
	...
	const addHandler = ( $element ) => {
		madxartworkFrontend.elementsHandler.addHandler( OsmMap, { $element, } );
	};
	madxartworkFrontend.hooks.addAction( 'frontend/element_ready/dce-osm-map.default', addHandler );
});
