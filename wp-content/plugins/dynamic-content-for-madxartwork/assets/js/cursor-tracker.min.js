(function($){var WidgetElements_CursorTrackerHandler=function($scope,$){var elementSettings=get_Dyncontel_ElementSettings($scope);var id_scope=$scope.attr('data-id');var cursorTarget=$scope.find('#cursors-'+id_scope);var init_cursortracker=function(){var progressTracker=elementSettings.cursortracker_scroll;let delay=elementSettings.delay.size;if(progressTracker){var progressPath=document.querySelector('.progress-wrap path.dce-cursortrack-path1');var pathLength=progressPath.getTotalLength();progressPath.style.transition=progressPath.style.WebkitTransition='none';progressPath.style.strokeDasharray=pathLength+' '+pathLength;progressPath.style.strokeDashoffset=pathLength;progressPath.getBoundingClientRect();progressPath.style.transition=progressPath.style.WebkitTransition='stroke-dashoffset 10ms linear';window.removeEventListener('scroll',updateProgress);var updateProgress=function(){var scroll=$(window).scrollTop();var height=$(document).height()-$(window).height();var progress=pathLength-(scroll*pathLength/height);progressPath.style.strokeDashoffset=progress};updateProgress();window.addEventListener('scroll',updateProgress)}
if(madxartworkFrontend.isEditMode()&&$('body > .cursors').length){$('body > .cursors').remove()}
