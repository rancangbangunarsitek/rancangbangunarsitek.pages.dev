...
const initializeHiddenLabelField = (field, form) => {
	...
	const hiddenField = form.getElementsByClassName('madxartwork-field-group-' + fieldId)[0];
	if (! hiddenField) {
...
const initializeHiddenLabelFields = ($form) => {
	$form.find('.madxartwork-field-type-hidden_label').each((_, f) => initializeHiddenLabelField(f, $form[0]));
};

jQuery(window).on('madxartwork/frontend/init', function() {
	madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', initializeHiddenLabelFields);
});
