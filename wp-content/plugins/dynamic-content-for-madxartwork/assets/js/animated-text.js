(
	function ( $ ) {
		var WidgetElements_AnimateTextHandler = function ( $scope,$ ) {
			...
			var changeText = function () {
				if ( currentTextIndex < texts.length - 1 ) {
					...
					currentTextIndex++;
					target.removeClass( 'madxartwork-repeater-item-' + ids[ oldTextIndex ] );
					target.html( texts[ currentTextIndex ] );
					target.addClass( 'madxartwork-repeater-item-' + ids[ currentTextIndex ] );
			...
			var animSplitText = function () {
				...
				if ( oldTextIndex < 0 ) {
					target.html( texts[ 0 ] );
					target.addClass( 'madxartwork-repeater-item-' + ids[ 0 ] );
				}
		...
		$( window ).on( 'madxartwork/frontend/init',function () {
			madxartworkFrontend.hooks.addAction( 'frontend/element_ready/dyncontel-animateText.default',
