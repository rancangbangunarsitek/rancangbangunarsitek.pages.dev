...
const getAllFields = ($form, fieldIds) => {
	...
	for (const id of fieldIds) {
		const $group = $form.find('.madxartwork-field-group-' + id);
		if ( $group.length ) {
...
const initializeStepsJumping = ($form) => {
	...
	let $steps = $form.find('.madxartwork-field-type-step');
	if ($steps.length < 3) {
	...
	for (const step of $steps) {
		...
		const $next = $step.find('.madxartwork-field-type-next button');
		const $previous = $step.find('.madxartwork-field-type-previous button');
		steps.push({
			nextButton: $next,
...
const getOnFormChange = ({fields, field_conditions, submit_conditions, $form, lang}) => {
	const $submitButton = $form.find('.madxartwork-field-type-submit');
...
function initializeConditionalFields($form) {
	const $outWrapper = $form.find('.madxartwork-form-fields-wrapper');
...
jQuery(window).on('madxartwork/frontend/init', function() {
	madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', initializeConditionalFields);
	// We delay adding the hook so it is run after the steps ares initialized:
	madxartworkFrontend.hooks.addAction('frontend/element_ready/global', () => {
		madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', initializeStepsJumping);
	});
});
