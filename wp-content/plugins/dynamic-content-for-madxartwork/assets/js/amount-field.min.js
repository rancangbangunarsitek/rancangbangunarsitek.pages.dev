...
let refresher=refresherGenerator(dceAmountField.makeGetFieldFunction(form));let onChange=()=>{let result=refresher();hiddenInput.value=result;visibleInput.value=textBefore+result+textAfter;if("createEvent" in document){var evt=document.createEvent("HTMLEvents");evt.initEvent("change",!1,!0);hiddenInput.dispatchEvent(evt)}else{hiddenInput.fireEvent("onchange")}}
onChange();form.addEventListener('input',onChange)}
function initializeAllAmountFields($scope){$scope.find('.madxartwork-field-type-amount').each((_,w)=>initializeAmountField(w,$scope[0]))}
jQuery(window).on('madxartwork/frontend/init',function(){if(madxartworkFrontend.isEditMode()){return}
madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default',initializeAllAmountFields)})