(function ($) {
    ...
    class Smooth {
        ...
        setWidth() {
            heightAdminBar = 0;
            if ($('body').is('.admin-bar') && !madxartworkFrontend.isEditMode()) {
                heightAdminBar = 45;
            }
    ...
    var init_Scrollify = function ( ) {
        ...
        if (settings_page.custom_class_section_sfy) {
            ...
            $customClass = 'madxartwork-section:not(.madxartwork-inner-section)';
        }

        $target_sections = '.madxartwork-' + currentPostId;
        if (!$target_sections)
            $target_sections = '';

        sezioni = $target_sections + '.madxartwork .madxartwork-section-wrap .' + $customClass;
        wrapperSezioni = $($target_sections + '.madxartwork .madxartwork-section-wrap');
        ...
        $.scrollify({
            ...
            afterRender: function () {
                is_scrollify = true;
                if (settings_page.enable_scrollify_nav || madxartworkFrontend.isEditMode()) {
    ...
    var init_InertiaScroll = function () {
        ...
        if ($('.madxartwork-template-canvas').length) {
            main = document.querySelector($target_sections);
            mainWrap = document.querySelector($target_sections + '.madxartwork .madxartwork-section-wrap');
        } else {
    ...
    $(window).on('madxartwork/frontend/init', function () {
       ...
       if (typeof madxartworkFrontendConfig.settings.page !== 'undefined') {
           ...
           currentPostId = madxartworkFrontendConfig.post.id;
           ...
           if (madxartworkFrontend.isEditMode()) {
               if (madxartwork) {
                   ...
                   madxartwork.settings.page.addChangeCallback('enable_dceScrolling', handlescroll_viewport);
                   ...
                   madxartwork.settings.page.addChangeCallback('snapscroll_nav_style', handleScrollify_navstyle);
                   madxartwork.settings.page.addChangeCallback('snapscroll_nav_title_style', handleScrollify_titlestyle);

                   // ScrollEffects
                   madxartwork.settings.page.addChangeCallback('enable_scrollEffects', handleScrollEffects);
                   madxartwork.settings.page.addChangeCallback('animation_effects', handleScrollEffects_animations);
                   madxartwork.settings.page.addChangeCallback('remove_first_scrollEffects', handleScrollEffects_removefirst);

                   // InertiaScroll
                   madxartwork.settings.page.addChangeCallback('enable_inertiaScroll', handleInertiaScroll);
                   madxartwork.settings.page.addChangeCallback('directionScroll', handleInertiaScroll_direction);

                   madxartwork.settings.page.addChangeCallback('coefSpeed_inertiaScroll', handleInertiaScroll);
                   madxartwork.settings.page.addChangeCallback('skew_inertiaScroll', handleInertiaScroll);
                   madxartwork.settings.page.addChangeCallback('bounce_inertiaScroll', handleInertiaScroll);
               }
           }
       }
   });
