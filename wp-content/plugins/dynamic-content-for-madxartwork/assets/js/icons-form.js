...
var iconsForm = ($scope, $) => {
	let form = $scope[0];
	var allInput = form.querySelectorAll(".madxartwork-form-fields-wrapper input");
	var allTextarea = form.querySelectorAll(".madxartwork-form-fields-wrapper textarea");
	var allSelect = form.querySelectorAll(".madxartwork-form-fields-wrapper select");
	var allLabels = form.querySelectorAll(".madxartwork-form-fields-wrapper .madxartwork-field-label");
	let icon;
	let inputHeight = jQuery('.madxartwork-form-fields-wrapper input').outerHeight();
	let labelHeight = jQuery('.madxartwork-field-label').outerHeight();
	let fontSize;
	let color;
	let margin;
	...
	allInput.forEach(function(field) {
		...
		if(icon) {
			...
			jQuery( '.madxartwork-field-label svg').addClass('label-icons');
	...
	allLabels.forEach(function(label) {
		...
		if(icon) {
			...
			if( ! labelIconSizeDefined ) {
				jQuery( '.madxartwork-field-label svg' ).css('height', labelHeight + 'px');
				jQuery( '.madxartwork-field-label svg' ).css('width', labelHeight + 'px');
				jQuery( '.madxartwork-field-label svg' ).css('margin-right', labelHeight / 4 + 'px');
			} else {
				jQuery( '.madxartwork-field-label svg' ).css('margin-right', labelIconSize / 4 + 'px');
			}
		}
	});
}
...
jQuery(window).on('madxartwork/frontend/init', function() {
	madxartworkFrontend.hooks.addAction('frontend/element_ready/form.default', iconsForm);
});
