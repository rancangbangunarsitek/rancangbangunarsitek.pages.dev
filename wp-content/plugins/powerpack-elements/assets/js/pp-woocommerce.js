(function ($) {
	...
	var ppSwiperSliderAfterinit = function ( $scope, carousel, carouselWrap, elementSettings, mySwiper, slider_type ) {
		...
		var $containers = {
			...
			".madxartwork-tabs": ".madxartwork-tab-title",
		};
	...
	var WidgetPPWooProducts = function ($scope, $, $panel) {
		...
		if (slider_wrapper.length > 0) {
			...
			jQuery(carouselWrap).closest('.madxartwork-widget-wrap').addClass('e-swiper-container');
			jQuery(carouselWrap).closest('.madxartwork-widget').addClass('e-widget-swiper');
	...
	.on( 'click', '.pp-woocommerce-pagination a.page-numbers', function( e ) {
		...
		$scope = $( this ).closest( '.madxartwork-widget-pp-woo-products' );
			...
			.on("added_to_cart.pp_cart", function (
				...
				if (
					$(this_button)
						...
						.parent()
						.parent()
						.hasClass("madxartwork-widget-pp-woo-add-to-cart")
				) {
					$btn = $(this_button);
	...
	var WooCategoriesHandler = function ($scope, $) {
		...
		jQuery(slider_wrapper).closest('.madxartwork-widget-wrap').addClass('e-swiper-container');
		jQuery(slider_wrapper).closest('.madxartwork-widget').addClass('e-widget-swiper');
	...
	$(window).on("madxartwork/frontend/init", function () {
		...
		//madxartworkFrontend.hooks.addAction('frontend/element_ready/pp-woo-product-tabs.default', WooProductTabsHandler);
	});
})(jQuery);
