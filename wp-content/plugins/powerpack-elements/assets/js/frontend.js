(function ($) {
    ...
    var ppSwiperSliderinit = function (carousel, carouselWrap, elementSettings, sliderOptions) {
		...
		$(carousel).closest('.madxartwork-widget-wrap').addClass('e-swiper-container');
		$(carousel).closest('.madxartwork-widget').addClass('e-widget-swiper');
    ...
    var CounterHandler = function ($scope, $) {
		...
		if ( 'undefined' !== typeof madxartworkFrontend.waypoint ) {
	...
	var tooltipsHandler = function ($scope) {
		...
		toolTopElm.each(function () {
            ...
            // tablet
            if ( window.innerWidth <= madxartworkBreakpoints.lg && window.innerWidth >= madxartworkBreakpoints.md ) {
                ttPosition = $scope.find('.pp-pricing-table-tooptip[data-tooltip]').data('tooltip-position-tablet');
            }

            // mobile
            if ( window.innerWidth < madxartworkBreakpoints.md ) {
                ...
                $scope.on( 'click', '.pp-gallery-filter', function() {
					...
					$(galleryItems).each(function() {
						...
						if ( lightboxLibrary === 'fancybox' ) {
							...
							imgLink.attr('data-madxartwork-lightbox-slideshow', filterIndex + '_' + widgetId);
						}
					});
        ...
        var carousel             = $scope.find( '.pp-showcase-preview' ).eq( 0 ),
            ...
            if ( scrollableNav === 'yes' ) {
                ...
                $nav_wrap.slick({
                    ...
                    responsive:     [
                        {
                        breakpoint: madxartworkBreakpoints.lg,
                            settings: {
                        ...
                        {
                        breakpoint: madxartworkBreakpoints.md,
                            settings: {
    ...
    var TimelineHandler = function ( $scope, $ ) {
		...
		if ( isEditMode ) {
			settings.window = madxartwork.$previewContents;
		}
    ...
    var CardSliderHandler = function ($scope, $) {
		...
		if ( 'no' !== elementSettings.open_lightbox ) {
			$scope.find('.pp-card-slider-item.swiper-slide-duplicate').each(function () {
				...
				lightboxItem.removeAttr( 'data-madxartwork-open-lightbox data-madxartwork-lightbox-slideshow data-madxartwork-lightbox-index' );
				lightboxItem.removeClass( 'madxartwork-clickable' );
			});
		}
    ...
    $(window).on('madxartwork/frontend/init', function () {
		...
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/global', WrapperLinkHandler );
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/widget', ElementsTooltipHandler );
    });
