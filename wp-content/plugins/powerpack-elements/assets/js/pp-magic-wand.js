( function ( $ ) {
	...
	var item_type_madxartwork_hook = [ 'widget', 'column', 'section', 'container' ];
	...
	PPMWHandler = {
		...
		paste: function( data, element ) {
			...
			switch( data.elType ) {
				...
				case 'container':
					...
					container = madxartwork.getPreviewContainer();
		...
		createElement: function( targetElement, targetElementType, data ) {
			...
			if ( undefined == element ) {
				if ( "widget" === targetElementType ) {
					if ( targetElement.$el.next( '.undefined.madxartwork-widget-empty' )  ) {
						targetElement.$el.next( '.undefined.madxartwork-widget-empty' ).after( '<div class="madxartwork-alert madxartwork-alert-warning">' + pp_magic_wand.widget_not_found + '</div>' );
					}
				} else {
					if ( targetElement.$el.find( '.undefined.madxartwork-widget-empty' ) ) {
						targetElement.$el.find( '.undefined.madxartwork-widget-empty' ).after( '<div class="madxartwork-alert madxartwork-alert-warning">' + pp_magic_wand.widget_not_found + '</div>' );
					}
				}
			}
		...
		generateUniqueID: function( elements ) {		
			elements.forEach( function( item, index ) {
				if ( typeof madxartworkCommon.helpers.getUniqueId() != "undefined" ) {
					item.id = madxartworkCommon.helpers.getUniqueId();
				}
	...
	if ( 'undefined' !== typeof madxartwork ) {
		...
		item_type.forEach( function( item, index ) {
			madxartwork.hooks.addFilter( 'elements/' + item_type_madxartwork_hook[index] + '/contextMenuGroups', function ( groups, element ) {
