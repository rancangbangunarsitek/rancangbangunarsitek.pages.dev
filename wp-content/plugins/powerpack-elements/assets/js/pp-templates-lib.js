;( function( madxartwork, $, window ) {
	...
	var TemplateLibraryTemplateView = Marionette.ItemView.extend({
		className: function className() {
			...
			classes += ' madxartwork-template-library-template-remote';
	...
	var TemplateLibraryTemplatePowerPackView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-madxartwork-template-library-template-pp',
		ui: function ui() {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-madxartwork-template-library-header-preview-pp',
	...
	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-madxartwork-template-library-templates-pp',
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('madxartwork-templates-filter-active', !!(pp_templates_lib.templates.getFilter('text') || pp_templates_lib.templates.getFilter('favorite')));
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-madxartwork-templates-modal__header__logo_pp',
		className: function className() {
			var classes = 'madxartwork-templates-modal__header__logo';
			return classes;
		},
		events: function events() {
			return {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-madxartwork-template-library-header-actions-pp',
	...
	var TemplateLibraryLayoutView = madxartworkModules.common.views.modal.Layout.extend({
		...
		getTemplateActionButton: function(templateData) {
			var viewId = '#tmpl-madxartwork-template-library-insert-button';
			if ( templateData.isPro && 'inactive' === templateData.proStatus ) {
				viewId = '#tmpl-madxartwork-template-library-get-pro-button-pp';
			}
			if ( templateData.isPro && 'license_inactive' === templateData.proStatus ) {
				viewId = '#tmpl-madxartwork-pro-template-library-activate-license-button-pp';
			}
			//viewId = madxartwork.hooks.applyFilters('madxartwork/editor/pp-template-library/template/action-button', viewId, templateData);
				...
				'templates/blocks': {
					...
					getFilter: function() {
						return {
							source: 'powerpack',
							type: 'block',
							subtype: madxartwork.config.document.remoteLibrary.category,
						}
					}
				},
				'templates/pages': {
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			madxartworkCommon.ajax.addRequest('pp_get_library_data', ajaxOptions);
		};
		...
		this.showErrorDialog = function( errorMessage ) {
			...
			self.getErrorDialog()
				.setMessage( madxartwork.translate( 'templates_request_error' ) + '<div id="madxartwork-template-library-error-info">' + errorMessage + '</div>' )
				.show();
		};
	...
	madxartwork.on('document:loaded', function() {
		var $previewContents = madxartwork.$previewContents;
	
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.madxartwork-add-section-area-button.pp-add-template-button { margin-left: 8px; vertical-align: bottom; }';
			btnStyle += '.madxartwork-add-section-area-button.pp-add-template-button img { height: 40px; }';
		...
		$templateLibBtn.addClass( 'madxartwork-add-section-area-button pp-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add PowerPack Template' );
		$templateLibBtn.html( '<img src="' + pp_templates_lib.logoUrl + '" />' );
		$templateLibBtn.insertAfter( $previewContents.find('.madxartwork-add-section-area-button.madxartwork-add-template-button') );
		$templateLibBtn.on( 'click', function() {
			$e.run('powerpack/open');
		});

		$previewContents.find('body').delegate('.madxartwork-editor-element-add', 'click', function(e) {
			setTimeout(function() {
				var sectionInner = $previewContents.find('body').find('.madxartwork-add-section-inner');
				...
				if ( sectionInner.length > 0 ) {
					...
					$templateLibBtn.insertAfter( sectionInner.find('.madxartwork-add-section-area-button.madxartwork-add-template-button') );
				}
			}, 100);
		});
	});

} )( madxartwork, jQuery, window );