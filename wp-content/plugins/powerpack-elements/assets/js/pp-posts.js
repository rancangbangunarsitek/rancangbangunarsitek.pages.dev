( function( $ ) {
	...
	var PostsHandler = function( $scope, $ ) {
		...
		if ( container.find('.pp-posts').hasClass( 'pp-posts-skin-checkerboard' ) && $scope.hasClass( 'pp-equal-height-yes' ) ) {
			function setEqualHeight() {
				...
				if ($(window).width() >= madxartworkBreakpoints.md) {
					$height = maxHeight;
				}
		...
		if ( 'carousel' == layout ) {
			...
			$(carouselWrap).closest('.madxartwork-widget-wrap').addClass('e-swiper-container');
			$(carouselWrap).closest('.madxartwork-widget').addClass('e-widget-swiper');
			...
			if ( carousel.length > 0 ) {
				if ( 'undefined' === typeof Swiper ) {
					var asyncSwiper = madxartworkFrontend.utils.swiper;
	...
	$( document ).on( 'click', '.pp-post-load-more', function( e ) {
		...
		$scope = $( this ).closest( '.madxartwork-widget-pp-posts' );
		loader = $scope.find( '.pp-posts-loader' );

		//Coupon widget
		var $coupon_scope = $( this ).closest( '.madxartwork-widget-pp-coupons' );
	...
	var _callAjax = function( $scope, $obj, $append, $count ) {
		...
		$.ajax({
			...
			if ( $scope.find( '.madxartwork-invisible' ).length > 0 ) {
				$scope.find( '.madxartwork-invisible' ).removeClass( 'madxartwork-invisible' );
			}
		} );
	}
	...
	$( window ).on( 'madxartwork/frontend/init', function () {
		...
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.classic', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.card', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.checkerboard', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.creative', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.event', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.news', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.portfolio', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.overlap', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.template', PostsHandler );
		
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/pp-coupons.default', PostsHandler );
