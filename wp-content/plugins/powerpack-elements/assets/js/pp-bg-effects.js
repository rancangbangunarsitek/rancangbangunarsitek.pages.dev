(function ($) {
    ...
    var BgHandler = function ($scope, $) {
		...
		var bg_row_elem    = madxartworkFrontend.isEditMode() ? $scope.find('.pp-background-wrapper') : $scope,
		...
		var prepend_content  = function( canvas_opacity ) {
			if ( ! madxartworkFrontend.isEditMode() ) {
				...
				if ( $scope.find( '.madxartwork-background-overlay' ).length == 0 ) {
					bg_row_elem.prepend( content );
				} else {
					content.insertBefore( bg_row_elem.find('.madxartwork-container') );
				}
			} else {
				var bg_overlay = $scope.find( '.madxartwork-element-overlay ~ .madxartwork-background-overlay' ),
		...
		var animation_all_particles = function() {
			var partColor         = ( '' !== bg_row_elem.data( 'part-color' ) ) ? bg_row_elem.data( 'part-color' ) : '#BAB9B9',
			...
			if ( 'particles' === animation_type ) {
				var lineLinked 		= true,
					...
					partMoveRand 	= false,
					partSizeRandom	= true,
					lineColor       = ( '' !== bg_row_elem.data( 'line-color' ) ) ? bg_row_elem.data( 'line-color' ) : '#BAB9B9',
			...
			if ( 'nasa' === animation_type ) {
				var lineLinked		= false,
					...
					lineColor       = ( 'grab' === part_hover_effect && '' !== bg_row_elem.data( 'line-h-color' ) ) ? bg_row_elem.data( 'line-h-color' ) : '#BAB9B9',
					partHoverSize	= ( '' !== part_hover_size ) ? part_hover_size : 0;
			}
			...
			if ( 'snow' === animation_type ) {
				var partColor         = ( '' !== bg_row_elem.data( 'part-color' ) ) ? bg_row_elem.data( 'part-color' ) : '#DEF4FD'
					lineLinked		= false,
					partShapeType	= 'circle',
					partMoveRand 	= false,
					partSizeRandom	= true,
					...
					showHoverEffect	= ( 'noeffect' === part_hover_effect ) ? false : true,
					partHoverEffect	= ( 'none' !== part_hover_effect ) ? part_hover_effect : 'bubble',
					lineColor       = ( 'grab' === part_hover_effect && '' !== bg_row_elem.data( 'line-h-color' ) ) ? bg_row_elem.data( 'line-h-color' ) : '#DEF4FD',
    ...
    $( window ).on('madxartwork/frontend/init', function () {
		madxartworkFrontend.hooks.addAction('frontend/element_ready/global', BgHandler);
    });
