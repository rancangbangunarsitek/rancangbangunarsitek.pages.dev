(function ($) {
	var PPToCHandler = madxartworkModules.frontend.handlers.Base.extend({
		stretchElement: null,
		...
		getDefaultSettings: function () {
			...
			return {
				selectors: {
					...
					container:
						'.madxartwork:not([data-madxartwork-type="header"]):not([data-madxartwork-type="footer"])',
		...
		bindEvents: function bindEvents() {
			...
			if (elementSettings.sticky_toc_toggle) {
				madxartworkFrontend.elements.$window.on("resize", this.handleStickyToc);
			}
		...
		populateTOC: function populateTOC() {
			...
			$(window).on('scroll', function(){
				...
				if ("window_top" === elementSettings.scroll_to_top_option) {
					...
					var $id = self.getID().parents('.madxartwork-widget-wrap');
		...
		collapseOnInit: function collapseOnInit() {
			...
			if ( 0 !== minimizedOn.length  && "object" === typeof minimizedOn ) {
				minimizedOn.forEach(function (value) {
					if (
						( "desktop" === value[0] && "desktop" == currentDeviceMode && $(window).width() < madxartworkFrontend.config.breakpoints.xxl ) ||
						( "tablet" === value[0] && "tablet" === currentDeviceMode && $(window).width() < madxartworkFrontend.config.breakpoints.lg ) ||
						( "mobile" === value[0] && "mobile" === currentDeviceMode && $(window).width() < madxartworkFrontend.config.breakpoints.md )
					) {
						self.collapseBox();
					}
				});
		...
		stickyScroll: function (){
			...
			var target = this.getID();
			var elementSettings = this.getElementSettings();
			var item = document.querySelector(".madxartwork-widget-pp-table-of-contents");			
		...
		scrollToTop: function scrollToTop() {
			...
			if ("window_top" === scrollTo) {
				...
				var $id = this.getID().parents('.madxartwork-widget-pp-table-of-contents');
	...
	jQuery(window).on("madxartwork/frontend/init", function () {
		var addHandler = function ($element) {
			madxartworkFrontend.elementsHandler.addHandler(PPToCHandler, {
				$element: $element,
			});
		};
