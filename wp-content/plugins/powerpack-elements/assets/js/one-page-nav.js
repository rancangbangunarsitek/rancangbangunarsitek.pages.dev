(function ($) {
    ...
    var OnePageNavHandler = function ($scope, $) {
        ...
        function updateDot() {
            $('.madxartwork-section').each(function(){
                var $this = $(this);
                if ( ( $this.offset().top - $(window).height()/2 < $(window).scrollTop() ) && ( $this.offset().top >= $(window).scrollTop() || $this.offset().top + $this.height() - $(window).height()/2 > $(window).scrollTop() ) ) {
					...
					if ( $( '.pp-one-page-nav-item' )[0] === $( $active_item )[0] && 1 === $first_load ) {
						$( $active_item ).find( 'a' ).trigger( 'click' );
						$first_load = 0;
					}
                } else {
    ...
    $(window).on('madxartwork/frontend/init', function () {
        madxartworkFrontend.hooks.addAction('frontend/element_ready/pp-one-page-nav.default', OnePageNavHandler);
    });
