(function($) {
						...
						callback: function (response) {
							if ( response != '' ) {
								...
								if ('invisible' == self.data('validate')) {
									self.closest('.madxartwork-widget').find('.pp-submit-button').trigger('click');
								}
							}
						}
					});
	...
	PPRegistrationForm.prototype = {
		...
		_initReCaptcha: function() {
			...
			if ( madxartworkFrontend.isEditMode() && undefined == reCaptchaField.attr( 'data-widgetid' ) ) {
				onLoadPPReCaptcha();
			}
		},
		...
		_submit: function(e) {
			...
			var theForm			= this.form,
				...
				nodeId      	= theForm.closest( '.madxartwork-widget-pp-registration-form' ).data( 'id' );
				//nodeId      	= theForm.parents( '.madxartwork-element.madxartwork-widget' ).last().data( 'id' );
			...
			// Validate Required.
			var self = this;
			theForm.find('.pp-rf-field.madxartwork-mark-required').each(function() {
				var field    = $(this),
