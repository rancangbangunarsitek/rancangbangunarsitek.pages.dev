...
var Container = /*#__PURE__*/function (_ArgsObject) {
  ...
  function Container(args) {
    ...
    if (!_this.document) {
      _this.document = madxartwork.documents.getCurrent();
    }
  ...
  (0, _createClass2.default)(Container, [{
    ...
    value: function validateArgs(args) {
      ...
      if (false !== args.parent) {
        this.requireArgumentInstance('parent', madxartworkModules.editor.Container, args);
      }
    }
    ...
    value: function handleRepeaterChildren() {
      ...
      Object.values(this.controls).forEach(function (control) {
        ...
        _this5.repeaters[control.name] = new madxartworkModules.editor.Container({
      ...
      if (['widget', 'document'].includes(this.type)) {
        ...
        if (!this.model.get('supportRepeaterChildren') && 1 === repeaters.length) {
          Object.defineProperty(this, 'children', {
            get: function get() {
              madxartworkDevTools.deprecation.deprecated('children', '3.0.0', 'container.repeaters[ repeaterName ].children');
    ...
    value: function addRepeaterItem(repeaterName, rowSettingsModel, index) {
      ...
      if (!rowId) {
        rowId = 'bc-' + madxartworkCommon.helpers.getUniqueId();
        rowSettingsModel.set('_id', rowId);
      }
      this.repeaters[repeaterName].children.splice(index, 0, new madxartworkModules.editor.Container({
        ...
        label: this.label + ' ' + __('Item', 'madxartwork'),
    ...
    value: function findChildrenRecursive(callback) {
      madxartworkDevTools.deprecation.deprecated('container.findChildrenRecursive( callback )', '3.5.0', 'container.children.findRecursive( callback )');
      return this.children.findRecursive(callback);
    }
  }, {
    key: "forEachChildrenRecursive",
    value: function forEachChildrenRecursive(callback) {
      madxartworkDevTools.deprecation.deprecated('container.forEachChildrenRecursive( callback )', '3.5.0', 'container.children.forEachRecursive( callback )');
    ...
    value: function isEditable() {
      return 'edit' === madxartwork.channels.dataEditMode.request('activeMode') && 'open' === this.document.editor.status;
    }
  }, {
    key: "isDesignable",
    value: function isDesignable() {
      return madxartwork.userCan('design') && this.isEditable();
    }
    ...
    value: function getGlobalValue(name) {
      ...
      if (control.groupType) {
        ...
        var responsivePrefixRegex = madxartwork.breakpoints.getActiveMatchRegex();
        var propertyName = control.name.replace(control.groupPrefix, '').replace(responsivePrefixRegex, '');
        if (!data.value[madxartwork.config.kit_config.typography_prefix + propertyName]) {
          return;
        }
        propertyName = propertyName.replace('_', '-');
        value = "var( --e-global-".concat(control.groupType, "-").concat(id, "-").concat(propertyName, " )");
        if (madxartwork.config.ui.defaultGenericFonts && control.groupPrefix + 'font_family' === control.name) {
          value += ", ".concat(madxartwork.config.ui.defaultGenericFonts);
        }
      } else {
    ...
    value: function getGlobalDefault(controlName) {
      ...
      if (controlGlobalArgs !== null && controlGlobalArgs !== void 0 && controlGlobalArgs.default) {
        ...
        if (!madxartwork.config.globals.defaults_enabled[controlType]) {
          return '';
        }
...
BaseSettingsModel = Backbone.Model.extend({
  ...
  initialize: function initialize(data, options) {
    ...
    self.controls = madxartwork.mergeControlsSettings(options.controls);
    self.validators = {};
    if (!self.controls) {
      return;
    }
    ...
    _.each(self.controls, function (control) {
      ...
      if ('object' === (0, _typeof2.default)(control.default)) {
        defaults[controlName] = madxartworkCommon.helpers.cloneObject(control.default);
      } else {
      ...
      if (undefined !== attrs[controlName] && isMultipleControl && !_.isObject(attrs[controlName]) && !hasDynamicSettings) {
        madxartworkCommon.debug.addCustomError(new TypeError('An invalid argument supplied as multiple control value'), 'InvalidElementData', 'Element `' + (self.get('widgetType') || self.get('elType')) + '` got <' + attrs[controlName] + '> as `' + controlName + '` value. Expected array or object.');
  ...
  convertRepeaterValueToCollection: function convertRepeaterValueToCollection(attrs, repeaterControl) {
    return new Backbone.Collection(attrs[repeaterControl.name], {
      model: function model(attributes, options) {
        ...
        if (!attributes._id) {
          attributes._id = madxartworkCommon.helpers.getUniqueId();
        }
  ...
  getStyleControls: function getStyleControls(controls, attributes) {
    var self = this;
    controls = madxartworkCommon.helpers.cloneObject(self.getActiveControls(controls, attributes));
    var styleControls = [];
    jQuery.each(controls, function () {
      var _control$dynamic;
      var control = this,
        controlDefaultSettings = madxartwork.config.controls[control.type];
  ...
  getActiveControls: function getActiveControls(controls, attributes) {
    ...
    jQuery.each(controls, function (controlKey, control) {
      if (madxartwork.helpers.isActiveControl(control, attributes, controls)) {
  ...
  clone: function clone() {
    return new BaseSettingsModel(madxartworkCommon.helpers.cloneObject(this.attributes), madxartworkCommon.helpers.cloneObject(this.options));
  },
  ...
  parseDynamicSettings: function parseDynamicSettings(settings, options, controls) {
    var self = this;
    settings = madxartworkCommon.helpers.cloneObject(settings || self.attributes);
    options = options || {};
    controls = controls || this.controls;
    jQuery.each(controls, function () {
      var control = this,
      ...
      if (undefined === dynamicSettings) {
        dynamicSettings = madxartwork.config.controls[control.type].dynamic;
      }
      ...
      var dynamicValue;
      try {
        dynamicValue = madxartwork.dynamicTags.parseTagsText(valueToParse, dynamicSettings, madxartwork.dynamicTags.getTagDataContent);
      } catch (error) {
        if (madxartwork.dynamicTags.CACHE_KEY_NOT_FOUND_ERROR !== error.message) {
          throw error;
        }
        dynamicValue = '';
        ...
        madxartwork.dynamicTags.refreshCacheFromServer(function () {
  ...
  parseGlobalSettings: function parseGlobalSettings(settings, controls) {
    ...
    jQuery.each(controls, function (index, control) {
      ...
      if (undefined === globalSettings) {
        globalSettings = madxartwork.config.controls[control.type].global;
      }
  ...
  toJSON: function toJSON(options) {
    ...
    return madxartworkCommon.helpers.cloneObject(data);
  }
});
...
var _default = /*#__PURE__*/function (_madxartworkModules$Mod) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initDialog() {
      ...
      this.getDialog = function () {
        if (!dialog) {
          ...
          dialog = madxartworkCommon.dialogsManager.createWidget(settings.dialogType, settings.dialogOptions);
    ...
    value: function setViewed() {
      ...
      madxartworkCommon.ajax.addRequest('introduction_viewed', {
        data: {
...
var EditorModule = madxartworkModules.Module.extend({
  onInit: function onInit() {
    var _this = this;
    var $window = jQuery(window);
    $window.on('madxartwork:init-components', this.onmadxartworkInitComponents.bind(this));
    $window.on('madxartwork:loaded', function () {
      _this.onmadxartworkLoaded();
      madxartwork.on('document:loaded', _this.onDocumentLoaded.bind(_this));
    });
    $window.on('madxartwork:init', this.onmadxartworkReady);
  },
  ...
  onmadxartworkReady: function onmadxartworkReady() {
    this.onmadxartworkInit();
    madxartwork.on('frontend:init', this.onmadxartworkFrontendInit.bind(this)).on('preview:loaded', this.onmadxartworkPreviewLoaded.bind(this));
  }
});
EditorModule.prototype.onmadxartworkLoaded = function () {};
EditorModule.prototype.onmadxartworkInit = function () {};
EditorModule.prototype.onmadxartworkPreviewLoaded = function () {};
EditorModule.prototype.onDocumentLoaded = function () {};
EditorModule.prototype.onmadxartworkFrontendInit = function () {};
EditorModule.prototype.onmadxartworkInitComponents = function () {};
...
var ControlsPopover = /*#__PURE__*/function () {
  function ControlsPopover(child) {
    ...
    this.$popover = jQuery('<div>', {
      class: 'madxartwork-controls-popover'
    });
  ...
  (0, _createClass2.default)(ControlsPopover, [{
    ...
    value: function createPopoverHeader() {
      ...
      var $popoverToggleControl = this.$popover.prev(),
        ...
        $resetLabel = $popoverToggleControl.find('.madxartwork-control-popover-toggle-reset-label');
      this.$popoverHeader = jQuery('<div>', {
        class: 'e-group-control-header'
      }).html('<span>' + __('Typography', 'madxartwork') + '</span>');
    ...
    value: function createAddButton() {
      ...
      this.$addButton.tipsy({
        title: function title() {
          return __('Create New Global Font', 'madxartwork');
        },
...
ControlsStack = Marionette.CompositeView.extend({
  classes: {
    popover: 'madxartwork-controls-popover'
  },
  activeTab: null,
  activeSection: null,
  className: function className() {
    return 'madxartwork-controls-stack';
  },
  templateHelpers: function templateHelpers() {
    return {
      elementData: madxartwork.getElementData(this.model)
    };
  },
  ...
  ui: function ui() {
    return {
      ...
      reloadButton: '.madxartwork-update-preview-button'
    };
  },
  ...
  behaviors: {
    HandleInnerTabs: {
      behaviorClass: __webpack_require__(/*! madxartwork-behaviors/inner-tabs */ "../assets/dev/js/editor/elements/views/behaviors/inner-tabs.js")
    }
  },
  ...
  initCollection: function initCollection() {
    this.collection = new Backbone.Collection(_.values(madxartwork.mergeControlsSettings(this.getOption('controls'))));
  },
  ...
  getNamespaceArray: function getNamespaceArray() {
    return [madxartwork.getPanelView().getCurrentPageName()];
  },
  openActiveSection: function openActiveSection() {
    ...
    if (activeSectionView[0]) {
      activeSectionView[0].$el.addClass('madxartwork-open');
      var eventNamespace = this.getNamespaceArray();
      eventNamespace.push(activeSection, 'activated');
      madxartwork.channels.editor.trigger(eventNamespace.join(':'), this);
    }
  },
  onRenderCollection: function onRenderCollection() {
  ...
  onDeviceModeChange: function onDeviceModeChange() {
    if ('desktop' === madxartwork.channels.deviceMode.request('currentMode')) {
      this.$el.removeClass('madxartwork-responsive-switchers-open');
    }
  },
  onChildviewControlSectionClicked: function onChildviewControlSectionClicked(childView) {
    var isSectionOpen = childView.$el.hasClass('madxartwork-open');
  ...
  onChildviewResponsiveSwitcherClick: function onChildviewResponsiveSwitcherClick(childView, device) {
    if ('desktop' === device) {
      this.$el.toggleClass('madxartwork-responsive-switchers-open');
    }
  }
}, {
