...
(self["webpackChunkmadxartwork"] = self["webpackChunkmadxartwork"] || []).push([["frontend-modules"],{
...
class _default extends madxartworkModules.ViewModule {
  ...
  getDocumentSettings(setting) {
    ...
    if (this.isEdit) {
      elementSettings = {};
      const settings = madxartwork.settings.page.model;
  ...
  runElementsHandlers() {
    this.elements.$elements.each((index, element) => setTimeout(() => madxartworkFrontend.elementsHandler.runReadyTrigger(element)));
  }
  onInit() {
...
module.exports = madxartworkModules.ViewModule.extend({
  ...
  __construct(settings) {
    ...
    this.isEdit = this.$element.hasClass('madxartwork-element-edit-mode');
    if (this.isEdit) {
  ...
  findElement(selector) {
    ...
    return $mainElement.find(selector).filter(function () {
      // Start `closest` from parent since self can be `.madxartwork-element`.
      return jQuery(this).parent().closest('.madxartwork-element').is($mainElement);
    });
  },
  ...
  initEditorListeners() {
    ...
    ['page'].forEach(function (settingsType) {
      ...
      if (self[listenerMethodName]) {
        self.editorListeners.push({
          event: 'change',
          to: madxartwork.settings[settingsType].model,
          callback(model) {
  ...
  addEditorListeners() {
    ...
    this.getEditorListeners().forEach(function (listener) {
      madxartworkFrontend.addListenerOnce(uniqueHandlerID, listener.event, listener.callback, listener.to);
    });
  },
  removeEditorListeners() {
    var uniqueHandlerID = this.getUniqueHandlerID();
    this.getEditorListeners().forEach(function (listener) {
      madxartworkFrontend.removeListeners(uniqueHandlerID, listener.event, null, listener.to);
    });
  },
  getElementType() {
  ...
  getElementSettings(setting) {
    ...
    if (this.isEdit && modelCID) {
      const settings = madxartworkFrontend.config.elements.data[modelCID],
        attributes = settings.attributes;
      let type = attributes.widgetType || attributes.elType;
      if (attributes.isInner) {
        type = 'inner-' + type;
      }
      let settingsKeys = madxartworkFrontend.config.elements.keys[type];
      if (!settingsKeys) {
  ...
  getEditSettings(setting) {
    var attributes = {};
    if (this.isEdit) {
      attributes = madxartworkFrontend.config.elements.editSettings[this.getModelCID()].attributes;
    }
    return this.getItems(attributes, setting);
  },
  getCurrentDeviceSetting(settingKey) {
    return madxartworkFrontend.getCurrentDeviceSetting(this.getElementSettings(), settingKey);
  },
  onInit() {
...
var _nestedTabs = _interopRequireDefault(__webpack_require__(/*! madxartwork/modules/nested-tabs/assets/js/frontend/handlers/nested-tabs */ "../modules/nested-tabs/assets/js/frontend/handlers/nested-tabs.js"));
...
module.exports = madxartworkModules.ViewModule.extend({
  getDefaultSettings() {
    return {
      element: null,
      direction: madxartworkFrontend.config.is_rtl ? 'right' : 'left',
      selectors: {
        container: window
      }
    };
...
class NestedTabs extends _base.default {
  ...
  activateTab(tabIndex) {
    ...
    $requestedContent[settings.showTabFn](animationDuration, () => {
      madxartworkFrontend.elements.$window.trigger('madxartwork-pro/motion-fx/recalc');
      madxartworkFrontend.elements.$window.trigger('madxartwork/nested-tabs/activate', $requestedContent);
    });
  ...
  bindEvents() {
    ...
    madxartworkFrontend.elements.$window.on('madxartwork/nested-tabs/activate', this.reInitSwipers);
  }
  ...
  reInitSwipers(event, content) {
    const swiperElements = content.querySelectorAll(`.${madxartworkFrontend.config.swiperClass}`);
  ...
  changeActiveTab(tabIndex) {
    ...
    if (fromUser && this.isEdit) {
      return window.top.$e.run('document/repeater/select', {
        container: madxartwork.getContainer(this.$element.attr('data-id')),
  ...
  createMobileTabs(args) {
    ...
    if (madxartworkFrontend.isEditMode()) {
      ...
      if ($removed.length) {
        return madxartworkModules.ViewModule.prototype.onInit.apply(this, args);
      }
    }
  }
}
