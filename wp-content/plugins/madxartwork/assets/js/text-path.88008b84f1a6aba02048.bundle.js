...
"use strict";
(self["webpackChunkmadxartwork"] = self["webpackChunkmadxartwork"] || []).push([["text-path"],{
...
class TextPathHandler extends madxartworkModules.frontend.handlers.Base {
  getDefaultSettings() {
    return {
      selectors: {
  ...
  setText(newText) {
    ...
    if (this.shouldReverseText()) {
      ...
      clone.id += '-clone';
      clone.classList.add('madxartwork-hidden');
  ...
  isRTL() {
    ...
    let isRTL = madxartworkFrontend.config.is_rtl;
    if (direction) {
      isRTL = 'rtl' === direction;
    }
    return isRTL;
  ...
  shouldReverseText() {
    ...
    const isFirefox = madxartworkFrontend.utils.environment.firefox;
    if (isFirefox) {
      return false;
    }
    const isChromium = madxartworkFrontend.utils.environment.blink;
    if (isChromium) {
