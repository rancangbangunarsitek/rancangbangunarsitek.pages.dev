...
export default function ImportContentDisplay( {
	...
	if ( ! kitData.length && hasPlugins ) {
		return (
			<Notice color="info" label={ __( 'Note:', 'madxartwork' ) }>
	...
	if ( ! kitData.length ) {
		return (
			<Notice color="danger">
				{ __( 'You can’t use this Website Kit because it doesn’t contain any content, pages, etc. Try again with a different file.', 'madxartwork' ) } <InlineLink onClick={ onResetProcess }>{ __( 'Go Back', 'madxartwork' ) }</InlineLink>
			</Notice>
		);
	}
	...
	return (
		<>
			{
				! isAllRequiredPluginsSelected &&
				<Notice color="warning" label={ __( 'Required plugins are still missing.', 'madxartwork' ) } className="e-app-import-content__plugins-notice">
					{ __( "If you don't include them, this kit may not work properly.", 'madxartwork' ) } <InlineLink url="/import/plugins">{ __( 'Go Back', 'madxartwork' ) }</InlineLink>
				</Notice>
			}
