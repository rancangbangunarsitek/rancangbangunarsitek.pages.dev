...
class TemplateTypesContext extends React.Component {
	...
	render() {
		if ( this.state.error ) {
			return <div className="e-loading-wrapper"><h3>{ __( 'Error:', 'madxartwork' ) } { this.state.error }</h3></div>;
		}
			...
			return (
				<div className="madxartwork-loading">
					<div className="madxartwork-loader-wrapper">
						...
						<div className="madxartwork-loading-title">{ __( 'Loading', 'madxartwork' ) }</div>
					</div>
				</div>
			);
		}
