...
import CssGrid from 'madxartwork-app/ui/atoms/css-grid';
		...
		<ModalProvider toggleButtonProps={ toggleButtonProps } title={ props.title }>
			<CssGrid columns={ 2 } spacing={ 60 }>
				<section>
					...
					<p>
						{ props.content }<br />
						<Button text={ __( 'Learn More', 'madxartwork' ) } color="link" target="_blank" url={ props.docs } />
					</p>
					<div className="eps-modal__tip">
						<h3>{ __( 'Tip', 'madxartwork' ) }</h3>
						<p>{ props.tip }</p>
					</div>
				</section>
				<section>
					<h3>{ __( 'Watch Video', 'madxartwork' ) }</h3>
